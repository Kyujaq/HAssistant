...
class OvernightOrchestrator:
    ...
    async def run_overnight_cycle(self):
        ...
        try:
            ...
            # Task 6: Kitchen Stack pipeline
            logger.info("Running Kitchen Stack pipeline...")
            try:
                kitchen_results = await self.kitchen_crew.run_kitchen_pipeline()
                results["tasks_completed"].append("kitchen_pipeline")
                results["kitchen_pipeline_results"] = kitchen_results
            except Exception as e:
                logger.error(f"Kitchen Stack pipeline failed: {e}")
                results["tasks_failed"].append("kitchen_pipeline")
                results["errors"].append(str(e))
