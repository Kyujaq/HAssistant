...
def add_item(item_data: Dict[str, Any]) -> int:
    ...
    try:
        ...
        cursor.execute("""
            INSERT INTO items (
                name, brand, category, unit, calories_per_unit, 
...
def get_item(item_id: int) -> Optional[Dict[str, Any]]:
    """
    Retrieve an item by its ID, including its images.
...
def add_item_image(item_id: int, image_path: str) -> int:
    """
    Adds a new image reference for an item.
    ...
    Returns:
        int: The ID of the newly added image record.
    ...
    try:
        ...
        raise sqlite3.Error(f"Failed to add item image: {e}")


def get_item_images(item_id: int) -> List[str]:
    """
    Retrieves all image paths for a given item.
    ...
    try:
        ...
        raise sqlite3.Error(f"Failed to get item images: {e}")
...
def get_current_inventory() -> List[Dict[str, Any]]:
    """
    Retrieve the current inventory of all items with a quantity greater than 0.
    This joins items and batches to provide a full overview.

    Returns:
        List[Dict[str, Any]]: A list of dictionaries, each representing a batch of an item.
    ...
    try:
        ...
        cursor.execute("""
            ...
            JOIN items i ON b.item_id = i.item_id
            WHERE b.quantity > 0
            ORDER BY i.name, b.expiration_date ASC;
        """)
        ...
        raise sqlite3.Error(f"Failed to get current inventory: {e}")
