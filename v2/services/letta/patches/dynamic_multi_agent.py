...
class DynamicMultiAgent(BaseAgent):
    ...
    async def step_stream(
        ...
        self.step(input_messages=input_messages)
        yield MessageStreamStatus.done
    ...
    def load_manager_agent(self) -> Agent:
        for participant_agent_id in self.agent_ids:
            participant_agent_state = asyncio.run(
                self.agent_manager.get_agent_by_id_async(agent_id=participant_agent_id, actor=self.user)
            )
